...
class LtoList4Parser : LotteryParser {
    companion object {
        private const val URL = "https://www.pilio.idv.tw/lto/list4.asp?indexpage="
        ...
        return Single.create { emitter ->
            FirebaseRemoteConfig.getInstance().fetchAndActivate().addOnCompleteListener {
                try {
                    emitter.onSuccess(
                        if (it.isSuccessful) {
                            FirebaseRemoteConfig.getInstance().getString("url_lto_list4")
                        } else {
                            URL
                        }
                    )
    ...
    private fun numberConverter(numbers: String): List<Int> {
        return numbers.substring(0, 7).split(" ").map { it.trim().toInt() }.toList()
    }
